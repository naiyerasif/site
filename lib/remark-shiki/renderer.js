...
const renderToHtml = (lines, options) => {
  ...
  if (options.langId || options.caption) {
    ...
    if (options.caption) {
      html += `<span class="code-caption">${options.caption}</span>`
    }
    
    html += `</div>`
  }

  html += `<pre class="shiki" tabindex="-1" style="background-color: ${bg}">`
  html += `<code tabindex="0">`
